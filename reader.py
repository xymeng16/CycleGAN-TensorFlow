...
class Reader():
  def __init__(self, tfrecords_file, image_width=256, image_height=256
  ...
  def _preprocess(self, image):
    image = tf.image.resize_images(image, size=(self.image_width, self.image_width))
    image = utils.convert2float(image)
    image.set_shape([self.image_width, self.image_width, 3])
    return image
